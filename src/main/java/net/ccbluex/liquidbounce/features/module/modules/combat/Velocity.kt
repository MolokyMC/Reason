...
class Velocity : Module() {
    ...
    private val modeValue = ListValue("Mode", arrayOf("GrimFull","GrimAC","Cancel", "Simple", "AACv4", "AAC4Reduce", "AAC5Reduce", "AAC5.2.0", "AAC", "AACPush", "AACZero",
    ...
    */
    ...
    fun onUpdate(event: UpdateEvent) {
        if ((onlyGround.get() && !mc.thePlayer.onGround) || (onlyMove.get() && !MovementUtils.isMoving())) return
        ...
        when (modeValue.get().lowercase()) {
            "grimfull" ->{
                ...
                if(cancelPackets == 0){
                    if (modeValue.get() == "GrimFull"){
                        closeblink()
                    }
                }
            }
            "grimac" -> if (mc.thePlayer.hurtTime > 0) {
    ...
    fun onPacket(event: PacketEvent) {
        ...
        fun startBlink(){
            ...
            if (packet2 is C03PacketPlayer)
                event.cancelEvent()
            if (packet2 is C04PacketPlayerPosition || packet2 is C06PacketPlayerPosLook ||
                packet2 is C08PacketPlayerBlockPlacement ||
                packet2 is C0APacketAnimation || packet2 is C0FPacketConfirmTransaction ||
                packet2 is C0BPacketEntityAction || packet2 is C02PacketUseEntity || (packet2::class.java.simpleName.startsWith("C", true))
            ) {
                event.cancelEvent()
                packets.add(packet2)
            }
            if(packet2::class.java.getSimpleName().startsWith("S", true)) {
                if(packet2 is S12PacketEntityVelocity && (mc.theWorld?.getEntityByID(packet2.entityID) ?: return) == mc.thePlayer){return}
                event.cancelEvent()
                inBus.add(packet2 as Packet<INetHandlerPlayClient>)
            }
        }

        if (modeValue.equals("GrimFull")) {
            if (packet is S12PacketEntityVelocity && onlyGround.get() && mc.thePlayer.onGround) {
            ...
            if (packet is S12PacketEntityVelocity && onlyMove.get() && MovementUtils.isMoving()) {
                val packetEntityVelocity = packet
                if ((mc.theWorld?.getEntityByID(packetEntityVelocity.entityID) ?: return) != mc.thePlayer)
                    return
