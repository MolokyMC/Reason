...
import net.ccbluex.liquidbounce.utils.packet.CPacketPlayerBlockPlacement
...
class NoSlow : Module() {
    ...
    fun onMotion(event: MotionEvent) {
        ...
        when (modeValue.get().lowercase()) {
            "grimfull" -> {
                if((event.eventState == EventState.PRE && mc.thePlayer!!.itemInUse != null && mc.thePlayer!!.itemInUse!!.item != null) && mc.gameSettings.keyBindUseItem.isKeyDown && (mc.thePlayer!!.heldItem!!.item is ItemFood) || (mc.thePlayer!!.heldItem!!.item is ItemPotion) || (mc.thePlayer!!.heldItem!!.item is ItemSword) || (mc.thePlayer!!.heldItem?.item is ItemBucketMilk)){
                    val curSlot = mc.thePlayer!!.inventory.currentItem
                    val spoof = if (curSlot == 0) 1 else -1
                    mc.netHandler.addToSendQueue(C09PacketHeldItemChange(curSlot + spoof))
                    mc.netHandler.addToSendQueue(C09PacketHeldItemChange(curSlot))
                }
                if (event.eventState == EventState.PRE && (mc.thePlayer!!.heldItem!!.item is ItemSword && mc.gameSettings.keyBindUseItem.isKeyDown)) {
                    mc.netHandler.addToSendQueue(C08PacketPlayerBlockPlacement(mc.thePlayer!!.inventory.getCurrentItem()))
                    mc.netHandler.addToSendQueue(CPacketPlayerBlockPlacement(getHytBlockpos(),255,mc.thePlayer!!.heldItem,0f,0f,0f))
                }
            }
            "grimac" -> {
                if((event.eventState == EventState.PRE && mc.thePlayer!!.itemInUse != null && mc.thePlayer!!.itemInUse!!.item != null) && !mc.thePlayer!!.isBlocking && (mc.thePlayer!!.heldItem!!.item is ItemFood) || (mc.thePlayer!!.heldItem!!.item is ItemPotion)){
                ...
                if (event.eventState == EventState.PRE && (mc.thePlayer!!.heldItem!!.item is ItemSword)) {
                    ...
                    mc.netHandler.addToSendQueue(C08PacketPlayerBlockPlacement(mc.thePlayer!!.inventory.getCurrentItem() as ItemStack))
                }
            }
