...
@ModuleInfo(name = "AntiFakePlayer", spacedName = "Anti Fake Player", description = "Made and Fixed by RyF with ❤", category = ModuleCategory.PLAYER)
class AntiFakePlayer : Module() {
    ...
    fun onPacket(event: PacketEvent) {
        ...
        // 屏蔽Antigetname
        if (packet is S02PacketChat && packet.chatComponent.unformattedText.contains(
                ":"
    ...
    fun onAttack(e: AttackEvent){
        target = e.targetEntity as EntityLivingBase
    }
    ...
    fun onUpdate(event: UpdateEvent) {
        if (addBotOnKill.get()) {
            if (target != null && target!!.isDead) {
                playerDeathAction(target!!.name, addBotOnKillDelay.get().toLong())
                target = null
            }
        }
        if (bots < 0) bots = 0
