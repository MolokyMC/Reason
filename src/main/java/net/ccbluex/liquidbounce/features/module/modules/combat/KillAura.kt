...
class KillAura : Module() {
    ...
    private val airBypass = BoolValue("AirBypass", false)
    val groundRangeValue = FloatValue("GroundRange", 3.7f, 1f, 8f) { airBypass.get() }
    val airRangeValue = FloatValue("AirRange", 3.4f, 1f, 8f) { airBypass.get() }
    ...
    // Modes
    private val rotations = ListValue("RotationMode", arrayOf("Vanilla", "BackTrack", "Spin","HYTRotation","Down","None"), "BackTrack")
    ...
    private val airStopSprintValue = BoolValue("AirStopKeepSprint", true) { keepSprintValue.get() }

    // AutoBlock
    private val autoBlockModeValue = ListValue("AutoBlock", arrayOf("None", "Packet", "AfterTick", "NCP", "OldHypixel","GameSetting"), "None")
    ...
    fun onMotion(event: MotionEvent) {
        if (airStopSprintValue.get()){
            if (mc.thePlayer.isAirBorne){
                if (keepSprintValue.get()) keepSprintValue.set(false)
            }else{
                if (!keepSprintValue.get()) keepSprintValue.set(true)
            }
        }
    ...
    private fun getTargetRotation(entity: Entity): Rotation? {
        ...
        if (rotations.get().equals("Down", ignoreCase = true)){
            ...
            val (_, rotation) = RotationUtils.lockView_Down(
                boundingBox,
                outborderValue.get() && !attackTimer.hasTimePassed(attackDelay / 2),
                randomCenterValue.get(),
                predictValue.get(),
    ...
    private fun startBlocking(interactEntity: Entity, interact: Boolean) {
        ...
        if (autoBlockModeValue.get().equals("gamesetting", true)) {
            if (mc.thePlayer.heldItem.item is ItemSword) mc.gameSettings.keyBindUseItem.pressed = true
            blockingStatus = true
            return
        }
    ...
    private fun stopBlocking() {
        ...
        if (blockingStatus) {
            ...
            else if (autoBlockModeValue.get().equals("gamesetting", true)){
