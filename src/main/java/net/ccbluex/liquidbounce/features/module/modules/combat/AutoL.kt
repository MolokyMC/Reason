...
@ModuleInfo(name = "AutoL", spacedName = "Auto L", description = "AutoL", category = ModuleCategory.COMBAT)
class AutoL : Module() {
    ...
    private fun playerChat(name: String) {
        if (enablement.get()) {
            ...
            replaceFilterWords()
            if (chatTotalKill.get()) text =
                ("$text | " + suffixTextBeforeRecord.get() + kill + suffixTextAfterRecord.get())
